...
import { Search, Edit, Eye, Calculator, DollarSign, ArrowRight, FileText, Printer, Trash2 } from "lucide-react"
...
export default function BudgetPage() {
    ...
    if (existingPresupuesto && existingPresupuesto.id) {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          seña: parseFloat(budgetFormData.seña.replace(',', '.')) || 0,
        })
  ...
  const handleMoveToRepair = async (repair: Repair) => {
    try {
      ...
      toast({
        title: "Movido a reparación",
        description: `El presupuesto ${repair.numeroIngreso} ha sido movido a reparación.`,
      })
      
      // Refrescar la lista de presupuestos después de un breve delay
      setTimeout(() => {
        if (typeof window !== 'undefined') {
          window.location.reload();
        }
